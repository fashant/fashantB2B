...
function getAutocompleteData() {
    var jqXHR = $.ajax({
        type: 'post',
        url: 'http://demofashant.com/partials/parseSearch.php',
        dataType: "json",
        async: false,
        data: {autocomplete:document.getElementById('Location').value},
        success: function (msg) {
            // var string = JSON.stringify(msg);
            // alert($.parseJSON(string));
        }
    });

    return JSON.parse(jqXHR.responseText);
    //return $.parseJSON(JSON.stringify(jqXHR.responseText));
}

function updateFavorite(type, id){
    $.ajax({
        type: 'post',
        url: 'http://demofashant.com/partials/parseFavoriteBrands.php',
...
function openReviewComments(id){
...
function commentOnReview(id){
    $.ajax({
        type: 'post',
        url: 'http://demofashant.com/partials/parseReviewComment.php',
        data: $('#review_comment_form_' + id).serialize(),
        success: function (msg) {
            var result = msg.substr(0, msg.indexOf(' '));
            ...
            if(result === "success"){
                swal({
                    title:"Success!",
                    text:"You've successfully commented a the review.",
                    icon:"success",
                    timer: 3000
                }).then(function () {
                    window.location.reload(true);
...
$("#top-select-search").on('keyup', function (e) {
    if (e.keyCode == 13) { // ON KEYPRESS ENTER
        var value = document.getElementById('top-select-search').value;
        window.location.href = "http://demofashant.com/list.php?f=&q=" + value;
    }
});